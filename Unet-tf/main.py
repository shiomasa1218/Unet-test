...
def train(parser):
    ...
    # Initialize session
    gpu_config = tf.ConfigProto(gpu_options=tf.GPUOptions(per_process_gpu_memory_fraction=0.7),
                                log_device_placement=False, allow_soft_placement=True)
    sess = tf.InteractiveSession(config=gpu_config) if gpu else print("GPU 使われてねえぞおおお")## tf.InteractiveSession()
...
def get_parser():
    parser = argparse.ArgumentParser(
        ...
        description='This module demonsts image segmentation using U-Net.',
        add_help=True
    )
